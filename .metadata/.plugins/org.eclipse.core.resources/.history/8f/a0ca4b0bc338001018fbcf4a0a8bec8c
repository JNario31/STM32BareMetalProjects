...
void i2c1_init(void)
{
	...
	/*Set PB8 and PB9 output type to open drain required for open-drain*/
	GPIOB->OTYPER |=(1U<<8);
	GPIOB->OTYPER |=(1U<<9);
	...
	/*Set rise time */
	/*Max time a signal is allowed to take to from low to high */
...
void i2c1_byte_read(char saddr, char maddr, char* data) {
     ...
     /* Transmit slave address + Read (sets r/w bit to 1 for read) */
     I2C1->DR = saddr << 1 | 1;
